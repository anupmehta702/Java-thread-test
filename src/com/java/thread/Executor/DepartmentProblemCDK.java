...
public class DepartmentProblemCDK {
    ...
    Processing department M for paper P3 by thread pool-1-thread-1
    Processing department E for paper P2 by thread pool-1-thread-2
    Processing department C for paper P1 by thread pool-1-thread-2
    Processing department E for paper P1 by thread pool-1-thread-2
    Processing department C for paper P2 by thread pool-1-thread-2
     */
    ...
    /*Output
Adding event with department M for paper P2 by thread pool-1-thread-2
...
Adding event with department M for paper P3 by thread pool-1-thread-2
Adding event with department C for paper P1 by thread pool-1-thread-1
Adding event with department E for paper P2 by thread pool-1-thread-2
Adding event with department C for paper P2 by thread pool-1-thread-1
Adding event with department E for paper P1 by thread pool-1-thread-2
Department --> M Event Queue --> [Event{deptName='M', paper='P1'}, Event{deptName='M', paper='P2'}, Event{deptName='M', paper='P3'}]
Department --> E Event Queue --> [Event{deptName='E', paper='P1'}, Event{deptName='E', paper='P3'}, Event{deptName='E', paper='P2'}]
...
Finished processing all events for department -E (Did it in sorted order bcoz we used PriorityQueue and comparable based on paper .
...
class DepartmentThread implements Runnable {
    ...
    String departmentName;
    Queue<Event> departmentQueue = new PriorityQueue<>();
    ...
    public void print() {
        System.out.println("Department --> " + departmentName + " Event Queue --> " + departmentQueue);
    }
    ...
    public synchronized void run() {
        while (departmentQueue.peek() != null) {
            Event event = departmentQueue.remove();
            try {
                if (event.deptName == "M") {
                    Thread.sleep(4000);
